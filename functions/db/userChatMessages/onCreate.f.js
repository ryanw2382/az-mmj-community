...
exports = module.exports = functions.database.ref('/user_chat_messages/{senderUid}/{receiverUid}/{messageUid}').onCreate(event => {
  ...
  const updateReceiverChatMessage = receiverChatMessageRef.update(snapValues)
  const updateSenderChat = senderChatRef.update({
    unread: 0,
    lastMessage: lastMessage,
    lastCreated: snapValues.created
  })
  const updateReceiverChat = receiverChatRef.update({
  ...
  if (snapValues.authorUid !== receiverUid) {
    const payload = {
      notification: {
        ...
        click_action: `https://az-mmj-community.firebaseapp.com/chats/edit/${senderUid}`,
        tag: `chat`
      }
    }
