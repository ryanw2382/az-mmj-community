...
import DispensaryForm from '../../components/Forms/DispensaryForm';
...
const path='/dispensaries/';
const form_name='dispensary';
  ...
  render() {
    ...
    const menuList=[
      ...
      },
      {
        ...
        onClick: ()=>{setDialogIsOpen('delete_dispensary', true);}
      }
    ]
    ...
    return (
        <Activity
            ...
            title={intl.formatMessage({id: match.params.uid?'edit_dispensary':'create_dispensary'})}>
          ...
          <div style={{margin: 15, display: 'flex'}}>
            ...
            <FireForm
                ...
                onSubmitSuccess={(values)=>{history.push('/dispensaries');}}
                onDelete={(values)=>{history.push('/dispensaries');}}
                uid={match.params.uid}>
              <DispensaryForm />
            </FireForm>
          </div>
          <Dialog
              title={intl.formatMessage({id: 'delete_dispensary_title'})}
              actions={actions}
              modal={false}
              open={dialogs.delete_dispensary===true}
...
)(injectIntl(withRouter(withFirebase(muiThemeable()(Dispensary)))));
